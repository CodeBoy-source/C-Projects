 ...
 * Primeramente necesitamos obtener la longitud de la lista.
 * Una vez que tengamos eso tenemos que ir hasta la ultima posicion.
 * LUego imprimimos los valores y ahora tenemos que ir hasta la ultima-1
 * Para controlar-lo nos auxiliamos de una variable static de valor igual
 * a la longitud de la lista. Y volvemos a llamar la función de forma recursiva
 * y reduciendo en 1 la variable. Mientras esta no sea igual a 0;
 */
void List::RevShow() {
    Node* extra;
    extra = this -> content;
    static int cont = this->Length();
    if(cont>0){
